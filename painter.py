...
while True:
    ...
    if(len(landmark_list)!=0):
        ...
        if (my_fingers[1] and not my_fingers[2]): # index == up, middle == not up
            ...
            if xp == 0 and yp == 0: # if not started drawing i.e. when cursor is at default == top left corner
                xp = x1
                yp = y1
    ...
    img_gray = cv2.cvtColor(image_canvas, cv2.COLOR_BGR2GRAY) # colorful -> black n' white (grayscale)
